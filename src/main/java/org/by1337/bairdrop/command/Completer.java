...
public class Completer implements TabCompleter {
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (sender instanceof Player pl) {
            ...
            if (args.length == 2 && args[0].equals("js"))
                return BAirDrop.getiConfig().getScripts().values().stream().map(File::getName).collect(Collectors.toList());
            if (args.length == 3 && args[0].equals("start"))
                return List.of(String.valueOf(pl.getLocation().getBlockX()));
            if (args.length == 4 && args[0].equals("start"))
                return List.of(String.valueOf(pl.getLocation().getBlockY()));
            if (args.length == 5 && args[0].equals("start"))
                return List.of(String.valueOf(pl.getLocation().getBlockZ()));
